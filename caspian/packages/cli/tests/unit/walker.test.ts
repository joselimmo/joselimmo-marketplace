...
describe("walker — symlink + realpath safety", () => {
  it("does not follow symlinks pointing outside cwd", async () => {
    ...
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "caspian-outside-"));
    try {
  ...
  it("flags files that resolve outside cwd via skippedOutsideCwd (cross-platform)", async () => {
    // Glob a pattern that would only match outside-cwd targets if any.
    // On all platforms, walking a tempdir against a different cwd should drop
    // matches whose realpath escapes cwd.
